...
namespace Tiled2ZXNext
{
    public class ProcessSceneSize : IProcess
    {
        ...
        public StringBuilder Execute()
        {
            ...
            sceneSize.Append("\t\tdb $").Append(blockType.ToString("X2")).Append("\t\t; data block type\r\n");
            sceneSize.Append("\t\tdw $").Append(2.ToString("X4")).Append("\t\t; Block size\r\n");
            sceneSize.AppendLine("\t\t; scene size width, height");
            sceneSize.Append("\t\tdb\t$").Append(width.ToString("X2")).Append(", $").AppendLine(height.ToString("X2"));
            return sceneSize;
        }
